...
# ---------------------------
# Класс ImageDataset (с сортировкой файлов)
# ---------------------------
class ImageDataset(data.Dataset):
    def __init__(self, root, transform=None, verbose=True):
        ...
        for dir_name in sorted(os.listdir(root)):
            ...
            count = 0
            for file_name in sorted(os.listdir(dir_path)):  # сортировка для детерминизма
    ...
    def __getitem__(self, idx):
        ...
        img = img.convert('RGB') # 3 канала для ResNet
        if self.transform:
...
# ---------------------------
# Класс EarlyStopping (с копией весов)
# ---------------------------
class EarlyStopping:
...
batch_size = 64
num_epochs = 100
learning_rate = 0.001          # для SGD можно немного увеличить lr
momentum = 0.9
weight_decay = 0.0001
random_seed = 10
...
# ---------------------------
# Разбиение на train/val/test с сохранением индексов
# ---------------------------
...
# Используем веса вместо pretrained
...
# Заменяем Adam на SGD для лучшей совместимости с DirectML
optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=momentum, weight_decay=weight_decay)

# Убираем verbose, так как он устарел
...
if resume and os.path.exists(checkpoint_path):
    ...
    # Восстановление генераторов случайных чисел
...
for epoch in range(start_epoch, num_epochs):
    ...
    # Получаем текущую learning rate (для вывода)
