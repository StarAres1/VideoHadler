...
def generate_enhanced_images(low_img):
    ...
    # Гамма-коррекция (12 уровней)
    gamma_values = np.linspace(0.4, 2.1, 12)
...
def main():
    ...
    parser.add_argument("--output", type=str, default="format_ssim_contrasts_19_1.jsonl", help="Выходной файл в формате JSON Lines (по умолчанию best_methods.jsonl)")
    parser.add_argument("--ext", type=str, default=".png", help="Расширение файлов (по умолчанию .png)")
    parser.add_argument("--start_num", type=int, default=291, help="Начинать обработку с изображений, чей номер референса не меньше указанного (по умолчанию 1)")
    parser.add_argument("--end_num", type=int, default=710, help="Заканчивать обработку на изображении с указанным номером (включительно). Если не задано, обрабатываются все от start_num и до конца.")
